...
import {
    Emotion,
    hex2rgb, ThemeBorder,
    ThemeColor, themeFactory,
    ThemeFont, ThemeGlobal, ThemeIcon,
    ThemeManager,
    ThemeScrollbar,
    ThemeShadow,
    ThemeSize
...
export const createTheme = (overrider:(manager:ThemeManager)=>void ) => (emotion: Emotion, manager: ThemeManager) => {
    const { css } = emotion;
...
export const getTheme = (overrider:(manager:ThemeManager)=>void) =>
    themeFactory((emotion, manager) => createTheme(overrider)(emotion, manager));
